...
* 파이썬이라는 언어를 이용하여 생물정보학이라는 학문을 배우고 있다.
* 파이썬의 클래스를 이용하여 객체 지향 프로그래밍(OOP, object-oriented programming)을 해보고 싶다.
* 일반적인 파이썬 클래스 설명 (예: `Student()`) 말고 생물정보학적이고 실용적인 예제를 찾고 있다.

기본적으로 생물정보학을 배우는 분들을 타겟으로 삼고 있기 때문에 DNA 혹은 RNA, 전사(transcription)나 번역(translation)과
같은 기본적인 생물학 및 유전학 관련 용어가 등장합니다. 이러한 용어들은 어느 정도 알고 계신다는 가정 하에 별도의 설명
없이 넘어가도록 하겠습니다.
