...
위해서는 매번 함수들을 불러와 사용하는 것이 번거로울 수 되는데요. 이러한 기능들이 모두 메소드로 구현되어 있어 사용하기
...
파이썬에서 클래스의 이름은 대문자로 시작하는 것을 권장이므로 특별한 이유가 없는 한 지켜주시는 것이 좋습니다.
...
### 서열에도 종류가 있는 법! DNA 와 RNA 를 구분해 봅시다.
...
dnaseq.reverse_compliment()  # Seq('TGTTC', 'dna')

rnaseq = Seq('GAACA', moltype='rna')
rnaseq.reverse_compliment()  # Seq('UGUUC', 'rna')
```
...
### 그때 그때 계산해서 알려준다: 프로퍼티 데코레이터 (@property)
...
하지만, 실제로 동등(`==`) 연산자를 이용해 비교해 보면 아래와 같이 예상과는 다른 결과를 내어줍니다.
...
class Seq(object):
    ...
    def __eq__(self, other):
        return (self._data == other._data) and (self._moltype == self._moltype)
